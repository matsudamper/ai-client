            ...
            BuiltinProjectId.Money -> {
                Info(
                    systemMessage = """
                        ...
                        時刻は全てOffsetなしとして扱ってください。
                        日時はISO8601形式で、日付と時刻の間は必ず「T」で区切ってください(例: "2025-03-20T15:30:00")。スペース区切りは不可です。
                        金額は合計・商品共に必ず税込みで出力してください。レシートの商品単価は税抜き(本体価格)で印字されていることが多いので、税抜きと判断したら税込みに換算してください。
                        税込みか税抜きかは「税抜」「本体」「外税」「内税」等の表記や、商品金額の合計と合計金額の関係から判断してください。
                        換算する場合は軽減税率対象(食品等。「軽」「※」「*」等のマークがあればそれに従う)は8%、それ以外は10%を掛けて、小数点以下は切り捨ててください。
                        出力後に商品の金額の合計が合計金額と大きくずれていないか検算し、ずれていれば税率の判断を見直してください。
                        以下のJSONフォーマットに従ってください。
                        ```json5
                        {
